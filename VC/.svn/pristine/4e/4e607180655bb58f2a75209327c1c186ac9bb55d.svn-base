...
public class DestroyAction extends TFAction {
	...
	public String queryDestroy() throws BusinessException {
		try{
			...
			Page resultPage = destroyService.queryDestroy(vcDestroy, page, rows, actionType);
	...
	public String initDestroyEdit() throws BusinessException {
		try{
			...
			List reuslt = destroyService.initDestroyView(Long.valueOf(id),strType);
	...
	public String saveDestroy() throws Exception {
		...
		try {
			...
			String realPath = SysConst.DESTROY_FILE_SAVE_PATH;
			...
			result = destroyService.saveDestroy(vcDestroy, vcDestroyDet, actionType, destroyStatus, file);
	...
	public String submitDestroy() throws BusinessException {
		try {
			...
			result = destroyService.executeSubmitDestroy(id, userCode, orgCode);
		}catch(BusinessException e){
			e.printStackTrace();
			this.setAjaxMessageInfo(TFStrUtils.getJSONErrorMessage(e.getMessage()));
			return NONE;
	...
	public String deleteDestroy() throws BusinessException {
		try {
			result = destroyService.deleteDestroy(id);
		}catch(BusinessException e){
			e.printStackTrace();
			this.setAjaxMessageInfo(TFStrUtils.getJSONErrorMessage(e.getMessage()));
			return NONE;
	...
	public String queryDestroyApprove() throws BusinessException {
		try{
			...
			Page resultPage = destroyService.queryDestroy(vcDestroy, page, rows, "approve");
	...
	public String initDestroyApprove() throws BusinessException {
		try{
			List reuslt = destroyService.initDestroyView(Long.valueOf(id),null);
			vcDestroy = (VcDestroy) reuslt.get(0);
			vcDestroyDet = vcDestroy.getVcDestroyDets();
			vcApprove = (List<VcApprove>) reuslt.get(1);
		}catch(BusinessException e){
	...
	public String approveDestroy() {
		try {
			...
			result = destroyService.executeApprove(vcApprove, id, actionType);
		}catch(BusinessException e){
			e.printStackTrace();
			this.setAjaxMessageInfo(TFStrUtils.getJSONErrorMessage(e.getMessage()));
			return NONE;
