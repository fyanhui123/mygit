	...
	initPage: function(){
		...
		$("#appPassBtn").click(function(){
			if($("#appEditForm").validate().form()){
				...
				common.ajaxSubmitDfCallBack(purchaseAppEdit.callBackReload, $.taHash.get("ctx")+"/orderManager/purchaseApprove.do?ajax=true&checkStatus=2", formStr);
			}else{
				return false;
			}
		});
		...
		$("#appUnPassBtn").click(function(){
			if($("#appEditForm").validate().form()){
				...
				$("#appUnPassBtn").attr("disabled",true);
				var formStr = $("#appEditForm").serialize();
				common.ajaxSubmitDfCallBack(purchaseAppEdit.callBackReload, $.taHash.get("ctx")+"/orderManager/purchaseApprove.do?ajax=true&checkStatus=1",formStr);
			}else{
				return false;
			}
		});
