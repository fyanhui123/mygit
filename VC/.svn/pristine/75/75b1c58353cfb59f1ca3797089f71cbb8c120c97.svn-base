var lostApplyEdit = {
		...
		lineDataArry : [ 
			...
			{title:"单证状态",fieldId:"docStatus",fieldName:"docStatus",fieldClass:"required",width:"60px",type:"3",disabled:"disabled",
				/*	dataFormat:[{selectedValue:"S1", data:[{"value":"S1","label":"印刷入库"},{"value":"S2","label":"申领/调拨入库"},{"value":"S3","label":"回收入库"},{"value":"A","label":"可使用"},{"value":"C1","label":"库存作废"}]
				}]*/
				dataFormat:[{
					selectedValue:"S2", data:[{"value":"C1","label":"审批退回"},{"value":"C1","label":"库存作废"},{"value":"S1","label":"印刷入库"},{"value":"S2","label":"申领/调拨入库"},{"value":"S3","label":"回收入库"}]
				}]
				},
		...
		initPage: function(){
			...
			function saveLostApply(lostStatus,butId){
				if($("#lostEditForm").validate().form()){
					...
					var action = $.taHash.get("ctx")+"/newinvoice/saveInvoiceLostApply.do";
