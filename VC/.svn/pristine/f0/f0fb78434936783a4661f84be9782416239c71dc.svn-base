var lostApproveQuery = {
	loadGridUrl : "/lostApplyJson/queryLostApprove.do",
	initPage : function() {
		/** 
			...
			// 获取选中行的id
				...
				if ("" != id) {
					...
					if (patt.test(id)) {
						...
						// ajax获取要修改的数据
						var action = $.taHash.get("ctx")+ "/lostApply/viewLostApply.do?ajax=true&actionType=approve&id=" + id;
		...
		$("#lostApplyGatherGrid").jqGrid( {
			...
			colModel : [ {
				...
				name : 'lostOrgCode',
				index : 'vcLost.lostOrgCode',
				autowidth : true,
				resizable : true,
				align : 'left'
			}, {
				name : 'lostOprCode',
				index : 'vcLost.lostOprCode',
				autowidth : true,
				resizable : true,
				align : 'left'
			}, {
