var destroy = {
	loadGridUrl : "/destroyJson/queryDestroy.do?ajax=true",
	initPage : function() {
		/**
			...
			// 获取选中行的id
				...
				if ("" != id) {
					...
					if (patt.test(id)) {
						...
						// ajax获取要修改的数据
					/*	var action = $.taHash.get("ctx")+ "/destroy/initDestroyEdit.do?ajax=true&actionType=modify&id="
						+ id
						common.ajaxGetData("destroyEditPage",
								action, null);
						// 调用dialog组件
						$('#destroyEditPage').dialog('open');*/
						
						common.localHref($.taHash.get("ctx")
								+ "/destroy/initDestroyEdit.do?actionType=modify&id="+id);
					}
				} else {
					alert("请选择一条记录！");
				}
			...
			// 获取选中行的id
				...
				if ("" != id) {
					...
					var action = $.taHash.get("ctx")
							+ "/destroyJson/deleteDestroy.do?ajax=true";
					if (confirm("确定删除?")) {
		...
		$("#submitDestroy").click(function() {
			// 获取选中行的id
				...
				if ("" != id) {
					...
					var action = $.taHash.get("ctx")
							+ "/destroyJson/submitDestroy.do?ajax=true";
					if (confirm("确定提交?")) {
						common.ajaxSubmitDfCallBack(destroy.callBack,
								action, param);
					}
	...
	/**
	 ...
	 viewDestroy : function(id){
		var action = $.taHash.get("ctx")+ "/destroy/initDestroyEdit.do?ajax=true&actionType=view&id=" + id;
