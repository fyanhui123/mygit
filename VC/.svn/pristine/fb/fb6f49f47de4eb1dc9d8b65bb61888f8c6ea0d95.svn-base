var validStorage = {
	 ...
	 */
	gridReload: function () {
		var gridReloadUrl = common.ctx+"/applyJson/queryValidStorageListByPages.do?ajax=true";
		...
		QueryCondition.validStatus=$("#validStatus").val();
		//在这个地方加个状态
	...
	initQueryPage: function(){
		...
		$("#selectStorageRec").click(function() {
			...
			if("" != id){
				...
				///销毁可以选择多条
				if(tableName!="destroyEditList" && tableName!="docIssuedInput"&& tableName!="lostApplyEditList" && isOverdue){
						if( patt.test(id)){
						    alert("只能选择一条记录！");
						}else{
						...
						for (var i=0;i<arrIds.length;i++){
							...
							if(tableName=="destroyEditList"){
								...
								if(curRowIndex!=rowIndex){	
									...
									$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "docVerCode", docVerCode);
									//$("#destroyEditList").mulLineWidget.resetVal(curRowIndex, "docVerCodeName", curDocVerName);
									$("#docVerCode__"+curRowIndex+"Name").val(curDocVerName);
								}
								$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "pressBatchNo", curPressBatchNo);
								$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "startNum", curStartNum);
								$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "endNum", curEndNum);
								$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "lostNum", curDocNum);
								$("#lostApplyEditList").mulLineWidget.resetVal(curRowIndex, "docStatus", curStatus);
							}
