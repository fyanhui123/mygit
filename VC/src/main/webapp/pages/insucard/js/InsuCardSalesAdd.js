		...
		initPage: function(){
	       //本级有效库存
			...
			$("#insuCardDetList").mulLineWidget({
				...
				columnData: insuCardSalesAdd.lineDataArry,
				exeFuction: insuCardSalesAdd.resetValP,
			...
			$("#saveBut").click(function(){
				//保存
				insuCardSalesAdd.clickButDeal("1","saveBut");
			});
			...
			$("#submitBut").click(function(){
				//提交
				insuCardSalesAdd.clickButDeal("2","submitBut");
			});
...
//****************add by chy 20131024 激活卡销售与销管交互改造*****begin****************
			...
			$("#saleOrgCode").change(function(){
				...
				//渠道类型根据销售机构代码获取
				$("#channelDetailCode").taAutoComplete({
					source : "/visa/baseinfoJson/queryChannelDetailCode.do?ajax=true&codeType=UnderWriteChannel&remark="+orgCode,
					minLength : 0,
					height : 210,
					width : 220
				});
				$("#channelDetailCode").removeAttr("disabled");
				...
				$("#insuCardSaleRecordVoSellerCode").val("");
				$("#insuCardSaleRecordVoSellerCodeName").val("");
				$("#channelDetailCode").val("");
				$("#channelDetailCodeName").val("");
			...
			$("#channelDetailCode").change(function(){
				...
				$("#businessSource").taAutoComplete({
					source : "/visa/baseinfoJson/queryChannelDetailCode.do?ajax=true&codeType=BusinessSource&remark="+channelDetailCode,
					minLength : 0,
					height : 210,
					width : 220
				});
			 	...
			 	$.ajax({
					...
					success: function(data){
						if(data != null && data != ""){
							...
							//MODIFY BY ZHXIAO3 VC-121激活卡激活卡未保存跟单业务员 begin
							$("#handlerCode").val(data.handlerCode);
...
//****************add by chy 20131024 激活卡销售与销管交互改造*****end****************			
		...
		clickButDeal:function(type,butid){
			if($("#insuCardSalesAddForm").validate().form()){
				 //  防止重复提交
				...
				//modify by zhxiao3 VC-126激活卡销售团队代码不再控制为必输项 begin
				var _businessSource = $("#businessSource").val();
				var _teamCode = $("#teamCode").val();
				if("" == _teamCode && "34" != _businessSource){
					$("#saveBut").removeAttr("disabled");
					$("#submitBut").removeAttr("disabled");
					alert("团队代码不能为空，请确认销管系统关联团队代码是否已失效！");
					return;
				}
				//modify by zhxiao3 VC-126激活卡销售团队代码不再控制为必输项 end
				...
				common.formSubmit("insuCardSalesAddForm", url);						
			}else{
				return false;
			}
		},
