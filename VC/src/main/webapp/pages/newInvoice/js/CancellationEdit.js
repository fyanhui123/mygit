var destroyEdit = {
	...
	lineDataArry : [ {
		...
		disabled:"disabled",
		dataFormat:[{
			selectedValue:"S2", data:[{"value":"C1","label":"库存作废"},{"value":"S1","label":"印刷入库"},{"value":"S2","label":"申领/调拨入库"},{"value":"S3","label":"回收入库"}]
		}]
	},
	...
	initPage : function() {
		...
		$("#saveBackBtn").click(function() {
			...
			common.localHref($.taHash.get("ctx")
					+ "/newDestroy/initDestroyQuery.do");
		});
		...
		function saveDestroy(destroyStatus,butId) {
			if ($("#destroyEditForm").validate().form()) {
			   ...
			   $("#submitBut").attr("disabled",true);
				...
				var action = $.taHash.get("ctx") + "/newDestroy/saveDestroy.do";
