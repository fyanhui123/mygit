var destroy = {
	loadGridUrl : "/newDestroyJson/queryDestroy.do?ajax=true",
	initPage : function() {
		/**
			...
			// 获取选中行的id
				...
				if ("" != id) {
					...
					/*	var action = $.taHash.get("ctx")+ "/destroy/initDestroyEdit.do?ajax=true&actionType=modify&id="
						...
						common.localHref($.taHash.get("ctx")
								+ "/newDestroy/initDestroyEdit.do?actionType=modify&id="+id);
					}
				} else {
					alert("请选择一条记录！");
				}
		...
		$("#submitDestroy").click(function() {
			// 获取选中行的id
				...
				if ("" != id) {
					...
					var action = $.taHash.get("ctx")
							+ "/newDestroyJson/submitDestroy.do?ajax=true";
					if (confirm("确定提交?")) {
						common.ajaxSubmitDfCallBack(destroy.callBack,
								action, param);
					}
