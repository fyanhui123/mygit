var destroyEdit = {
	...
	lineDataArry : [ {
		...
		dataFormat:[{
			selectedValue:"S2", 
			data:[{"value":"S1","label":"印刷入库"},{"value":"S2","label":"申领/调拨入库"},{"value":"S3","label":"回收入库"},{"value":"JX","label":"缴销"}]
		}]
	},
	...
	initPage : function() {
		...
		$("#saveBackBtn").click(function() {
			...
			common.localHref($.taHash.get("ctx")
					+ "/invoiceDestroy/destroyQuery.do");
		});
		...
		function saveDestroy(destroyStatus,butId) {
			if ($("#destroyEditForm").validate().form()) {
			   ...
			   $("#submitBut").attr("disabled",true);
				...
				var action = $.taHash.get("ctx") + "/invoiceDestroy/saveDestroy.do";
