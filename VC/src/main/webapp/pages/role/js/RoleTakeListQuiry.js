var roleTakerListInquiry = {
	loadGridUrl : "/roleJson/roleList.do?ajax=true",
		...
		$("#exportRoleListInquiry").click(function() {
			...
			common.ajaxSubmitDfCallBack(roleTakerListInquiry.exportHref, url, null);
		});
	    //机构选择
		...
		$("#rolelistquerys").click(function() {		
			roleTakerListInquiry.gridReload();
		});
		$("#rolelistquery").jqGrid( {
			url : $.taHash.get("ctx")+roleTakerListInquiry.loadGridUrl,
			mtype : "POST",
			datatype : "local",
			autowidth : true,
			height : 'auto',	
			colNames : ['机构代码','机构名称','用户名称','用户代码','员工代码','状态'],
			colModel : [ {
				name : 'companyCode',
				index : 'companyCode',
				width : '7%',
	...
	gridReload : function() {
		...
		var gridReloadUrl = $.taHash.get("ctx")+roleTakerListInquiry.loadGridUrl+"&"+tmpStr;
		...
		/*var tt=jQuery("#usertype option:selected").val();
		if(1==tt){
			setTimeout("roleTakerListInquiry.hiddenRelod()",1500); 
		}else if(0==tt){
			setTimeout("roleTakerListInquiry.showRelod()",1500); 
		}*/
	},
