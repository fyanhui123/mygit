var provide = {
	 ...
	 */
	gridReload: function () {
		var gridReloadUrl = $.taHash.get("ctx")+"/provideAllotJson/queryAllotListByPages.do?ajax=true";
		...
		*/		
		...
		if(objId == "edit"){
			...
			if(docVerCode==null ||docVerCode=="") {
				...
				$("#validStorageQuery_docVerCode").val(docVerCode);
				//申领机构
				var allotOrgCode=$("#applyOrgCode").val();
			...
			if(docVerCode==null || docVerCode==""){
				...
				setTimeout(function(){
					$.ajax({
						...
						dataType:'json',
						data:{applyId:allotId,docVerCode:docVerCode},
	...
	initEditPage:function() {
		...
		var lineData = [ 
		        		{title:"单证类型",fieldName:"docVerCode",fieldId:"docVerCode",fieldLableName:"docVerCodeName",width:"200px",type:"2",fieldClass:"required",
		        					dataFormat:[{source:"/visa/provideAllotJson/getJsonDocVersions.do?ajax=true&allotId="+applyId, height:"210px", width:'300px',minLength:0}]},
		...
		$("#saveButProvideEdit").click(function() {
			...
			if(valid) {
				var n = {};
				var allotDetList = $("#DocListProvide").mulLineWidget.getData();
				for(var i=0;i<allotDetList.length;i++){
					var st1=allotDetList[i].applyNum;
					var st2=allotDetList[i].provideNum;
					var startNum=allotDetList[i].startNum;
					var endNum=allotDetList[i].endNum;
					if(st1=='NaN'){
						alert('单证【'+allotDetList[i].docVerCode+'】申请数量为空！');
						return false;
					}
					if(st2=='NaN'){
						alert('单证【'+allotDetList[i].docVerCode+'】发放数量为空！');
						return false;
					}
					if(parseFloat(startNum)>parseFloat(endNum)){
						  alert('单证【'+allotDetList[i].docVerCode+'】起始流水大于终止流水！');
						 return false;
					}
					if(parseFloat(st1)<parseFloat(st2)){
					  alert('单证【'+allotDetList[i].docVerCode+'】发放数量大于申请数量！');
					 return false;
					}
					//add by chy 20130725 ,不同行的同一单证类型，发放数量总和不能大于申请数量 begin
					var _docVerCode = allotDetList[i].docVerCode;
					if(n[_docVerCode]){
				...
				//流水号区间是否重叠
				var isRepeat = common.checkOverLap(allotDetList, "docVerCode", "pressBatchNo", "startNum", "endNum");
				if(isRepeat){
					return false;
				}
				...
				var vcAllot = provide.getPageProvide();
				vcAllot.allotStatus="4";  //暂存状态
				
				provide.ajaxSaveProvide(vcAllot);
			} else {
				return;
			}					
		});
...
//			data:provide.applyQueryEnableStatus
		...
		*/
		...
		$("#provideBtn").click(function() {
			...
			if("" != id){
				...
				if(patt.test(id)){
					...
					//ajax获取要修改的数据
					common.ajaxGetData("editPageProvide", $.taHash.get("ctx")+"/provideAllot/queryProvideAllotInfo.do?id="+id+"&ajax=true&mngType=0&maxStoreType=maxStoreTime",null); 
		...
		$("#provideGrid").jqGrid({
			...
			autowidth : true,
			height: '210px',
			colNames : ['ID', '调拨单号', '调拨申请人', '调拨机构','调拨日期', '调拨单状态','applyStatus'],
			colModel : [{
				name : 'id',
				index : 'id',
				hidedlg : true,
			...
			*/
			...
			gridComplete: function(){
			    ...
			    for(var i=0;i < ids.length;i++){
			     ...
			     var editBtn = "<a href='#' onclick='provide.hrefView("+rowArray.id+")' style='text-decoration:underline;font-weight:normal;'>"+rowArray.allotCode+"</a>";
			     $("#provideGrid").jqGrid('setCell', ids[i], 'allotCode', editBtn);
			    }
		   }		
		});
	...
	ajaxSaveProvide:function(vcProvide) {
		...
		$.ajax({   
             type:'post',   
             url: $.taHash.get("ctx")+"/provideAllotJson/saveProvides.do?ajax=true",   
             dataType:'json',   
             data:params,
