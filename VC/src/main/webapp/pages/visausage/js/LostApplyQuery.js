var lostApply = {
	loadGridUrl : "/lostApplyJson/queryLostApply.do",
			...
			// 获取选中行的id
				...
				if ("" != id) {
					...
					if (patt.test(id)) {
						...
						$('#lostApplyEditPage').dialog('open');*/
						common.localHref($.taHash.get("ctx")
								+ "/lostApply/viewLostApply.do?actionType=modify&id="+id);
					}
				} else {
					alert("请选择一条记录！");
				}
		...
		$("#submitLostApply").click(function() {
			// 获取选中行的id
				...
				if ("" != id) {
					...
					var action = $.taHash.get("ctx")
							+ "/lostApplyJson/submitLostApply.do?ajax=true";
					if (confirm("确定提交?")) {
						common.ajaxSubmitDfCallBack(lostApply.callBack,
								action, param);
					}
